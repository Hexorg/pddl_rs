...
use super::{
    maps::Maps,
    AtomicFormula, BasicAction, Domain, Effect, Name, NegativeFormula,
    PreconditionExpr, Problem, Term, GD, StateUsize, CompiledActionUsize, for_all_type_object_permutations, Init, CompiledProblem, Instruction, CompiledAction, ASTActionUsize, FluentExpression,
};

mod first_pass;
use first_pass::{FirstPassContext, first_pass};
mod second_pass;
use second_pass::{SecondPassContext, second_pass};
...
pub struct Compiler<'ast, 'src> where 'ast:'src {
...
impl<'ast, 'src> Compiler<'ast, 'src> {
    pub fn new(domain:&'ast Domain<'src>, problem:&'ast Problem<'src>, maps:Maps<'src>) -> Self {
        Self { domain, problem, maps, pass1_data:FirstPassContext::new(domain.actions.len())}
    }
    pub fn compile(&mut self) -> Result<CompiledProblem, Vec<Error>> {
        let mut context = Context::new(ContextPass::First(FirstPassContext::new(self.domain.actions.len())));
...
fn visit_effect<'src, 'ast>(
    ...
    match effect {
       Effect::And(vec) => { 
        ...
        Effect::Assign(fterm, fexp) => todo!(),
        Effect::ScaleUp(fterm, fexp) => todo!(),
        Effect::ScaleDown(fterm, fexp) => todo!(),
        Effect::Increase(fterm, fexp) => {
            visit_fluent_expression(compiler, &fexp.1, context)?;
            match context.pass {
                ContextPass::Second(..) => second_pass::increate_function_term(compiler, fterm, context),
                _ => Ok(()),
            }
        },
        Effect::Decrease(fterm, fexp) => todo!(),
...
fn visit_fluent_expression<'src, 'ast>(
    ...
    match fexp {
        FluentExpression::Number(i) => if let ContextPass::Second(pass) = &mut context.pass {
...
fn visit_term_formula<'src, 'ast>(
    ...
    match context.pass {
        ContextPass::First(..) => first_pass::visit_term_formula(compiler, term_formula, args, context, formula),
        ContextPass::Second(..) => second_pass::visit_term_formula(compiler, term_formula, args, context, formula),
        _ => panic!()
    }
}
...
fn visit_name_formula<'src, 'ast>(
    ...
    match &mut context.pass {
        ContextPass::First(pass) => if context.is_negative {
        ...
        ContextPass::Second(pass) => {
            if let Some(offset) = pass.predicate_memory_map.get(formula) {
                ...
                if compiler.pass1_data.true_predicates.contains(formula) || compiler.pass1_data.false_predicates.contains(formula) {
                    // nop
                } else {
                    panic!()
                }
...
#[cfg(test)]
mod tests {
    use crate::{lib_tests::load_repo_pddl, compiler::{maps::map_objects, AtomicFormula, Name, Runnable, passes::{ContextPass, FirstPassContext}}, ReportPrinter};
    ...
    fn test_instructions() {
        ...
        let c_problem = match compiler.compile() {
            Ok(problem) => problem,
            Err(vec) => { for e in vec { Err(e.into()).unwrap_or_print_report(&sources) } panic!()}
        };
